...
async function vinterIndexAssetPriceTracker() {
  try {
    ...
    const weights = Object.values(weightObject);
...
async function vinterIndexRebalancer() {
  try {
    ...
    const weights = Object.values(rebalanceWeightObject);
  ...
  try {
    ...
    if (indexInfo) {
      ...
      if (storedRebalanceDate != latestRebalancingDate) {
        ...
        // Scheduling Jobs
        // Rebalancer
        let rebalanceCompositionPublishSchedule = new Date(latestRebalancingDate);
        rebalanceCompositionPublishSchedule.setDate(latestRebalancingDate.getDate() - 3); // T-3

        let rebalanceExecutionSchedule = new Date(latestRebalancingDate); // T

        let rebalanceCompositionPublishJob = schedule.scheduleJob(rebalanceCompositionPublishSchedule, vinterIndexRebalancer); // T-3
        debug(`RebalanceCompositionPublishJob scheduled for T-3: ${rebalanceCompositionPublishJob.nextInvocation()}`);

        let rebalanceExecutionJob = schedule.scheduleJob(rebalanceExecutionSchedule, vinterIndexRebalancer); // T
        debug(`RebalanceExecutionJob scheduled for T: ${rebalanceExecutionJob.nextInvocation()}`);
    ...
    debug('Error from RebalanceDateTracker:', error);
  }
};
