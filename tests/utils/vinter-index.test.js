...
const rebalanceWeightObject = vinterHourlyAPIResponse.data[0].rebalance_weights;
  ...
  it('vinterIndexAssetPriceTracker() should insert tokens, weights and prices into index collection', async () => {
    ...
    const expectedCurrentWeights = Object.values(weightObject).map((weight) =>
  ...
  it('vinterIndexRebalancer() should update the rebalanced assets, weights and prices', async () => {
    ...
    /// @dev T-3 scenario (rebal. info available but rebal. not executed)
   if(today < storedRebalanceDate) {
    // preparing expected values for T-3 scenario
    // Processing data as needed by the smart contracts
    const precision = 1000000000000000000; // 18 decimal places (1e18)
    expectedRebalanceAssets = Object.keys(rebalanceWeightObject);
    expectedRebalancedWeights = Object.values(rebalanceWeightObject).map((weight) =>
      Math.round(weight * precision)
    );
    ...
    expectedCurrentWeights = Object.values(rebalanceWeightObject).map((weight) =>
      Math.round(weight * precision)
    );
